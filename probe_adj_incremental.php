<?php
require_once "controladores/factus.controlador.php";
require_once "modelos/factus.modelo.php";

session_start();
$_SESSION['id'] = 14;

$auth = ControladorFactus::ctrAutenticar();
$token = $auth['token'];
$config = ModeloFactus::mdlObtenerConfiguracion();
$baseUrl = $config['api_url'];

$url = $baseUrl . "/v1/adjustment-notes/validate";

// Test 1: Range + support_document_id
echo "Test 1: Range + support_document_id ... ";
$payload = [
    "numbering_range_id" => 1193,
    "support_document_id" => 498
];
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, $url);
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
curl_setopt($ch, CURLOPT_POST, true);
curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode($payload));
curl_setopt($ch, CURLOPT_HTTPHEADER, ['Authorization: Bearer ' . $token, 'Content-Type: application/json', 'Accept: application/json']);
$resp = curl_exec($ch);
echo (strpos($resp, 'numbering_range_id') !== false ? "FAILED" : "OK") . "\n";
echo "   Errors: " . json_encode(json_decode($resp, true)['data']['errors'] ?? []) . "\n";

// Test 2: Adding correction_concept_code
echo "Test 2: Adding correction_concept_code ... ";
$payload['correction_concept_code'] = "2";
curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode($payload));
$resp = curl_exec($ch);
echo (strpos($resp, 'numbering_range_id') !== false ? "FAILED" : "OK") . "\n";
echo "   Errors: " . json_encode(json_decode($resp, true)['data']['errors'] ?? []) . "\n";

// Test 3: Adding billing_reference
echo "Test 3: Adding billing_reference ... ";
$payload['billing_reference'] = [
    "number" => "SEDS984000028",
    "uuid" => "4fed7d240941c93cd3c4d4c251f96b340c70db8f3a976de4b283593fd3600b610072e480ff133a1df7b5a91199e1a659",
    "issue_date" => "2026-02-24"
];
curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode($payload));
$resp = curl_exec($ch);
echo (strpos($resp, 'numbering_range_id') !== false ? "FAILED" : "OK") . "\n";
echo "   Errors: " . json_encode(json_decode($resp, true)['data']['errors'] ?? []) . "\n";

curl_close($ch);
?>