...
class imprimirDocumentoSoporte
{
    public $idDS;
    ...
    public function traerImpresionDetalle()
    {
        ...
        if (!$documentoSoporte) {
            die("Documento soporte no encontrado.");
        }
        ...
        if (!empty($documentoSoporte["qr_data"])) {
            ...
            $pdf->MultiCell(105, 0, $documentoSoporte["cuds"], 1, 'L', true, 1, 12, '', true);
        }

        // --- COLUMNA DERECHA (Totales) ---
        $tipoDescuento = $documentoSoporte["tipo_descuento"] ?? "porcentaje";
        $valorDescuento = floatval($documentoSoporte["valor_descuento"] ?? 0);
        ...
        if ($montoDescuento > 0) {
            $labelDesc = ($tipoDescuento == "porcentaje") ? "Descuento ($valorDescuento%):" : "Descuento:";
            $htmlRight .= '<tr><th>' . $labelDesc . '</th><td>-$' . number_format($montoDescuento, 2) . '</td></tr>';
        }
        ...
        ob_end_clean();
        $pdf->Output('documento-soporte-' . $documentoSoporte["numero_ds"] . '.pdf', 'I');
    }
}

if (isset($_GET["idDS"])) {
    $imprimir = new imprimirDocumentoSoporte();
    $imprimir->idDS = $_GET["idDS"];
