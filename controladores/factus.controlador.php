...
class ControladorFactus
{
	...
	static public function ctrActualizarConfiguracion()
	{
		if ((isset($_SERVER["REQUEST_METHOD"]) && $_SERVER["REQUEST_METHOD"] === "POST") || isset($_POST["apiUrl"]) || isset($_POST["csrf_token"])) {
			...
			$stmtCaja->bindParam(":base_conocimiento_activa", $baseConocimientoActiva, PDO::PARAM_INT);
			$stmtCaja->bindParam(":leads_whatsapp_activos", $leadsWhatsappActivos, PDO::PARAM_INT);
	...
	static public function ctrMostrarNotasCreditoServerSide($params)
	{
		...
		foreach ($notas as $value) {
			...
			// Permisos
			$puedoCrear = function_exists('puedeAccion') ? puedeAccion('notas_credito', 'crear') : true;
	...
	static public function ctrMostrarNotasAjusteDSServerSide($params)
	{
		...
		foreach ($notas as $key => $value) {
			...
			$puedoCrear = function_exists('puedeAccion') ? puedeAccion('notas_ajuste', 'crear') : true;
	...
	static public function ctrMostrarDocumentosSoporteServerSide($params)
	{
		...
		foreach ($data as $value) {
			...
			// 3. ACCIONES
			$puedoCrear = function_exists('puedeAccion') ? puedeAccion('documento_soporte', 'crear') : true;
